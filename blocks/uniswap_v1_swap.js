Blockly.Blocks["uniswap_v1_swap"] = {
  ...
  encoder: async function (value, tokenFrom, tokenTo) {
    if (tokenFrom == tokenTo)
      throw TypeError(
        "Error (uniswap_v1_swap): Cannot swap for the same token, please use different tokens from/to."
      );
    ...
    let exchange =
      tokenFrom == "0x0"
        ? await getExchange(tokenTo)
        : await getExchange(tokenFrom);
    ...
    if (tokenFrom == "0x0") {
      ...
      if (tokenTo == "0x0") {
        // from token to ether
        swapCalldata = uniswapExchangeInterface.functions.tokenToEthSwapInput.encode(
          [ethers.utils.parseEther(value), "1", deadline]
        );
      } else {
        // token to token
        swapCalldata = uniswapExchangeInterface.functions.tokenToTokenSwapInput.encode(
          [ethers.utils.parseEther(value), "1", "1", deadline, tokenTo]
        );
      }
