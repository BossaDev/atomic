...
function default_1() {
    const log = debug_1.default("buidler:core:tasks:run");
    config_env_1.task(task_names_1.TASK_RUN, "Runs a user-defined script after compiling the project")
        ...
        .setAction(async ({ script, noCompile }, { run, buidlerArguments }) => {
        if (!(await fs_extra_1.default.pathExists(script))) {
            throw new errors_1.BuidlerError(errors_list_1.ERRORS.BUILTIN_TASKS.RUN_FILE_NOT_FOUND, {
                script
            });
        }
        if (!noCompile) {
        ...
        try {
            process.exitCode = await scripts_runner_1.runScriptWithBuidler(buidlerArguments, script);
        }
        catch (error) {
            throw new errors_1.BuidlerError(errors_list_1.ERRORS.BUILTIN_TASKS.RUN_SCRIPT_ERROR, {
                script,
                error: error.message
            }, error);
        }
