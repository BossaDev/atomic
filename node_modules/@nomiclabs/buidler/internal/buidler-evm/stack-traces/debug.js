...
function printStackTrace(trace) {
    const withDecodedMessages = trace.map(entry => entry.type === solidity_stack_trace_1.StackTraceEntryType.REVERT_ERROR
        ? Object.assign({}, entry, { message: revert_reasons_1.decodeRevertReason(entry.message) }) : entry);
    const withHexAddress = withDecodedMessages.map(entry => "address" in entry
        ? Object.assign({}, entry, { address: ethereumjs_util_1.bufferToHex(entry.address) }) : entry);
    const withTextualType = withHexAddress.map(entry => (Object.assign({}, entry, { type: solidity_stack_trace_1.StackTraceEntryType[entry.type] })));
